<html>
    <body>
       ...
       <form method="POST" action="createCategoria.php"> 
...
<?php
    ...
    if(!empty($_POST)){
        ...
        if($validacao)
        {
            ...
            $q = $banco->prepare($sql);
            $q->execute(array($nome, $ultima_atualizacao));
            DB::disconnect();

            echo "<h3 style='color:green;position: relative;left:450px;'>Cadastro Realizado com Sucesso</h3>";
            echo "<a style='color:grey;position: relative;left:520px; font-size:16px;text-align:center;' href='tabelaCategoria.php'>Visualizar Tabela</a>";
        }
    }
?>
